...
class TriggerDetector:
    ...
    def parse_trigger_type(
        ...
        Returns:
            ...
            - trigger_author is the Jira username of the comment author that triggered
    ...
    def _find_last_trigger_author(
        ...
        """Find the author of the last comment containing the trigger marker.

        Args:
            comments: List of JiraComment objects (assumed ordered by creation time)
            trigger_marker: The trigger string to match

        Returns:
            Jira username of the author, or empty string if not found
        """
        author = ""
        for c in comments:
            if trigger_marker in c.body:
